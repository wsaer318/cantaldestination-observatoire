...
class TempTablesManager {
    ...
    public function __construct($silentMode = true) {
        ...
        $this->log_file = __DIR__ . '/data/logs/temp_tables_update.log';
    ...
    private function normalizeZone($zone_name) {
        ...
        // ✅ Mappings CORRIGÉS selon les nouvelles spécifications utilisateur
        // Ces mappings correspondent aux zones principales (non exclues) en base
        $mappings = [
            'HAUT CANTAL' => 'GENTIANE',                      // "Haut Cantal" → "GENTIANE" (zone principale)
            'HAUTES TERRES' => 'HTC',                         // "Hautes Terres" → "HTC" (zone principale)  
            'PAYS D\'AURILLAC' => 'CABA',                     // "Pays d'Aurillac" → "CABA" (zone principale)
            "PAYS D'AURILLAC" => 'CABA',                      // Variante apostrophe droite
            'PAYS D AURILLAC' => 'CABA',                      // Variante caractères spéciaux (CSV)
            'PAYS DAURILLAC' => 'CABA',                       // Variante sans espace
            'PAYS D�AURILLAC' => 'CABA',                      // Variante caractère spécial CSV (�)
            'LIORAN' => 'STATION',                            // "Lioran" → "STATION" (inchangé, correct)
            'CH�TAIGNERAIE' => 'CHÂTAIGNERAIE',              // Variante caractère spécial CSV (�) → zone existante
            'VAL TRUY�RE' => 'VAL TRUYÈRE',                   // Variante caractère spécial CSV (�) → zone existante
        ];
