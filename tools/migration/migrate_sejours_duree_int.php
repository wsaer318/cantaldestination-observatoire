<?php
/**
 * Migration spécifique des données de durée de séjour internationales
 * fact_sejours_duree_pays_temp -> fact_sejours_duree_pays
 */

require_once 'config/database.php';

echo "🚀 MIGRATION SPÉCIFIQUE - DURÉE DE SÉJOUR INTERNATIONALES\n";
echo "======================================================\n\n";
...
try {
    ...
    $temp_table = 'fact_sejours_duree_pays_temp';
    $main_table = 'fact_sejours_duree_pays';
    ...
    while (true) {
        ...
        $stmt = $db->prepare("
            SELECT date, id_zone, id_provenance, id_categorie, id_pays, id_duree, volume
            FROM $temp_table
            LIMIT ? OFFSET ?
        ");
    ...
    if ($total_migrated > 0) {
        echo "\n🔍 VÉRIFICATION DES DONNÉES MIGRÉES:\n";
        $result = $db->query("
            SELECT pays.nom_pays, COUNT(*) as nombre, SUM(vol.volume) as total_volume
            FROM $main_table vol
            JOIN dim_pays pays ON vol.id_pays = pays.id_pays
            GROUP BY vol.id_pays, pays.nom_pays
        ...
        $rank = 1;
        while ($row = $result->fetch_assoc()) {
            echo "  {$rank}. {$row['nom_pays']}: {$row['nombre']} enr. ({$row['total_volume']} volume)\n";
            $rank++;
        }
