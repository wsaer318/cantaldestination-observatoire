<?php
/**
 * API Bloc D2 Excursionnistes - Régions Optimisée avec Cache
 * Retourne les données des régions pour les excursionnistes
 */
...
// Vérifier le cache
$cachedData = $cacheManager->get('infographie_regions', $cacheParams);
if ($cachedData !== null) {
    header('X-Cache-Status: HIT');
    header('X-Cache-Category: infographie_regions');
...
try {
    ...
    // Calcul des plages de dates pour l'année courante et précédente (support override)
    ...
    foreach ($previousData as $row) {
        $previousIndex[$row['region_nouvelle']] = (int)$row['n_presences_n1'];
    }
    ...
    $cacheManager->set('infographie_regions', $cacheParams, $result);
    
    header('X-Cache-Status: MISS');
    header('X-Cache-Category: infographie_regions');
    // Appliquer les corrections si nécessaire
    if (isset($result['data'])) {
        $result['data'] = applyCorrectionIfNeeded($result['data'], $zone, $annee, $periode, 'infographie_regions_excursionnistes.php');
    }
