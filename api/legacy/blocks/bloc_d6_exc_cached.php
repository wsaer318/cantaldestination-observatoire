<?php
/**
 * API Bloc D6 Excursionnistes avec Cache Unifié - Âge
...
try {
    ...
    $cachedData = $cacheManager->get('tdb_age_excursionnistes', $cacheParams);
    if ($cachedData !== null) {
        header('X-Cache-Status: HIT');
        header('X-Cache-Category: tdb_age_excursionnistes');
    ...
    SELECT
        a.nom_age AS age,
        a.code_age AS code_age,
        SUM(f.volume) AS n_presences
    FROM fact_diurnes_age AS f
    JOIN dim_ages AS a ON f.id_age = a.id_age
    WHERE
        f.id_zone = @zone
        ...
        AND a.nom_age <> 'CUMUL'
        AND a.nom_age IS NOT NULL
        AND a.nom_age <> ''
    GROUP BY
        a.nom_age, a.code_age
    ORDER BY
        n_presences DESC
    LIMIT ?";
    ...
    foreach ($previousData as $row) {
        $previousIndex[$row['age']] = (int)$row['n_presences_n1'];
    }
    ...
    foreach ($currentData as $row) {
        ...
        $nPresencesN1 = $previousIndex[$age] ?? 0;
    ...
    $cacheManager->set('tdb_age_excursionnistes', $cacheParams, $result);
    ...
    error_log("API Âge Excursionnistes Error: " . $e->getMessage());
}
?> 
