...
if (!empty($_GET['space_id'])) {
    ...
    if (preg_match('/\/api\/shared-spaces\/infographics\/(\d+)/', $path, $matches)) {
        ...
        error_log("API Infographics - Test pattern 2: /\/" . getBasePath() . "\/api\/shared-spaces\/infographics\/(\d+)/");
        if (preg_match('/\/' . preg_quote(getBasePath(), '/') . '\/api\/shared-spaces\/infographics\/(\d+)/', $path, $matches)) {
            $spaceId = (int)$matches[1];
